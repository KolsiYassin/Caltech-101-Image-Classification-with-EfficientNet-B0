...
Training runs can be monitored with TensorBoard.

Start TensorBoard from the project directory:

```bash
tensorboard --logdir=runs
```

For a remote server accessed through SSH, use:

```bash
tensorboard --logdir=runs --bind_all
```

and forward port `6006` through SSH:
...
TensorBoard logs include training and validation loss, accuracy, and learning rate for experiments that have TensorBoard logging enabled.
...
pip install numpy matplotlib scikit-learn tensorboard
```

For GPU training, install the appropriate CUDA-enabled PyTorch version for your system. See the official [PyTorch installation guide](https://pytorch.org/get-started/locally/).
...
The training script saves checkpoints and training histories to the `results/` and `weights/` directories.
...
Evaluation results are saved as `test_metrics.npy` inside the corresponding experiment directory.
...
The same stratified dataset split is reused across experiments to ensure that different training configurations are evaluated on the same data.
